...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    Commands.sequence(
        ...
        indexer.set(0.5),
        linearIntake.setHeightAndStop(Meters.of(0.2)),
    ...
    Command shootCommand =
        (Commands.sequence(
            Commands.parallel(
                hood.setAngleAndStop(Degrees.of(25)), shooter.setVelocity(RPM.of(2500))),
            indexer.set(0.2)));

    Command noShootCommand =
        Commands.parallel(
            shooter.setVelocity(RPM.of(0)), hood.setAngle(Degrees.of(0)), indexer.set(0));

    driverController.button(1).onTrue(intakingFuel);
    driverController.button(2).whileTrue(shootCommand).whileFalse(noShootCommand);
