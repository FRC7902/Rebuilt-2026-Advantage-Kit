...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    Command intakingFuel = Commands.sequence(
      Commands.parallel(linearIntake.setHeightAndStop(Meters.of(0.05)),
      indexer.set(1.0),
      Commands.waitSeconds(5)
      ),
      linearIntake.setHeightAndStop(Meters.of(3.132)),
      indexer.set(0.05));
    
      driverController.button(1).onTrue(intakingFuel);

    Command shootingFuel = Commands.sequence(
      Commands.parallel(shooter.setVelocity(RPM.of(2500)),
      hood.setAngle(Degrees.of(25))),
      indexer.set(0.01)
    );

      driverController.button(2).whileTrue(shootingFuel);
  }
