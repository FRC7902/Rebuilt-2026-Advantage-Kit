...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    Command intakingFuel =
        Commands.sequence(
            Commands.parallel(
                linearIntake.setHeightAndStop(Meters.of(0.1)),
                indexer.set(1),
   ...
   Command shootingthefuel = Commands.parallel(
      shooter.setVelocity(RPM.of(2500)),
      hood.setAngleAndStop(Degrees.of(25)).andThen(indexer.set(1))
    );
    Command notshootingthefuel = Commands.parallel(
      shooter.setVelocity(RPM.of(0)),
      hood.setAngleAndStop(Degrees.of(0)),
      indexer.set(0)
    );
    driverController.button(2).whileTrue(shootingthefuel)
      .onFalse(notshootingthefuel);
  }
