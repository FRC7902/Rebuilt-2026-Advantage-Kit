...
/** AdvantageKit Linear Intake Subsystem, capable of replaying the linear intake. */
public class LinearIntakeSubsystem extends SubsystemBase {
  ...
  private final SparkMax linearIntakeFollowerMotor =
      new SparkMax(31, SparkLowLevel.MotorType.kBrushless);

  private final SmartMotorControllerConfig linearIntakeMotorConfig =
      new SmartMotorControllerConfig(this)
          ...
          // Follower motor follows primary linear intake motor with an inversed output.
          .withFollowers(Pair.of(linearIntakeFollowerMotor, true));

  private final SmartMotorController linearIntakeController =
      new SparkWrapper(linearIntakeMotor, motors, linearIntakeMotorConfig);

  private ElevatorConfig linearIntakeConfig =
      new ElevatorConfig(linearIntakeController)
          .withStartingHeight(Meters.of(0.5))
          .withHardLimits(Meters.of(0), Meters.of(3))
          .withTelemetry("LinearIntake", TelemetryVerbosity.HIGH)
          .withMass(weight);

  private final Elevator linearIntake = new Elevator(linearIntakeConfig);

  public LinearIntakeSubsystem() {
    new Trigger(() -> getPosition().lte(Meters.of(0.1)))
        .and(
            () ->
                linearIntakeInputs.setpoint.isEquivalent(
                    linearIntakeMotorConfig.convertFromMechanism(Rotations.of(0))))
        .whileTrue(linearIntake.set(0));
  }

  private void updateInputs() {
    linearIntakeInputs.setpoint =
        linearIntakeMotorConfig.convertFromMechanism(
            linearIntake.getMechanismSetpoint().orElse(Rotations.of(1)));
    linearIntakeInputs.position = linearIntake.getHeight();
    linearIntakeInputs.velocity = linearIntake.getVelocity();
    linearIntakeInputs.current = linearIntakeController.getStatorCurrent();
    linearIntakeInputs.volts = linearIntakeController.getVoltage();
  }
  ...
  public Command linearIntakeCmd(double dutyCycle) {
  ...
  public Command setPosition(Distance position) {
    Logger.recordOutput("LinearIntake/Setpoint", position);
    return linearIntake.setHeight(position);
  }
